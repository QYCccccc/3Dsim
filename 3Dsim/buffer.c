...
struct ssd_info *handle_write_buffer(struct ssd_info *ssd, struct request *req)
{
	...
	while (lpn <= last_lpn)       //lpn值在递增
	{
		...
		if (lpn == first_lpn)
		{
			...
			offset1 = ssd->parameter->subpage_page - (((lpn + 1)*secno_num_per_page - req->lsn)/secno_num_sub_page); //对于例子，offset1 = 0;
			state = state&(0xffffffff << offset1);
		}
		if (lpn == last_lpn)
		{												//9*32 = 288          256 + 24 = 280  LPN=8的页还剩下8个sector即一个sub_page
			offset2 = ssd->parameter->subpage_page - ((lpn + 1)*secno_num_per_page - (req->lsn + req->size)) / secno_num_sub_page;	//对于例子offset2 = 3,即最后一页使用了3个sub_page
			state = state&(~(0xffffffff << offset2));	//state = 0000_0111
		}
...
struct ssd_info * insert2buffer(struct ssd_info *ssd, unsigned int lpn, int state, struct sub_request *sub, struct request *req)
{
	...
	/*以LRU算法维持一个双向链表进行缓存结点的替换，
	将未在缓存中命中的数据写入缓存并将该节点提到LRU链表的头节点，
	将LRU链表尾结点替换形成子请求写回闪存。*/
	
	/*没有命中
	首先即要计算出free sector（表示还有多少可以直接写的buffer节点）。
	如果free_sector>=sector_count，即有多余的空间够lpn子请求写，不需要产生写回请求
	否则，没有多余的空间供lpn子请求写，这时需要释放一部分空间，产生写回请求。*/
	if (buffer_node == NULL)
	{
...
struct sub_request * creat_sub_request(struct ssd_info * ssd, unsigned int lpn, int size, unsigned int state, struct request * req, unsigned int operation)
{
	...
	**************************************************************************************/
	if (operation == READ)
	{
		loc = find_location(ssd, ssd->dram->map->map_entry[lpn].pn); //找到该LPN对应的物理位置
		sub->location = loc;
...
Status allocate_location(struct ssd_info * ssd, struct sub_request *sub_req)
{
	...
	{	//写子请求中的逻辑页状态与物理页状态不同，说明就是要更新该物理页的内容，而SSD不能重复写，所以要将数据读出，更新后再进行写入
		if ((sub_req->state&ssd->dram->map->map_entry[sub_req->lpn].state) != ssd->dram->map->map_entry[sub_req->lpn].state)
		{
			...
			update->state = ((ssd->dram->map->map_entry[sub_req->lpn].state^sub_req->state) & 0x7fffffff);   //得到更新写操作中新旧页面中不同的子页
