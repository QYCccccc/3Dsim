...
struct ssd_info *initiation(struct ssd_info *ssd)
{
	...
	secno_num_per_page = ssd->parameter->page_capacity / SECTOR;	//16kb / 512byte = 32
	secno_num_sub_page = ssd->parameter->subpage_capacity / SECTOR; //4096 / 512 = 8
...
struct blk_info * initialize_block(struct blk_info * p_block,struct parameter_value *parameter)
{
	...
	const double r_speed = Random_LogNormal(0, 1, 0.5, 1);
