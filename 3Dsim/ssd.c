...
//定义一个page有多少个扇区，一个subpage有多少个扇区
...
void tracefile_sim(struct ssd_info *ssd)
{
	...
	//进入这个预处理函数时，ssd->token没有初始化？
	//SSD预处理建立映射表
	rt_ssd_info = pre_process_page(ssd);
	if(rt_ssd_info == NULL)
	{
		return;
	}
...
struct ssd_info *process(struct ssd_info *ssd)
{
	...
	**********************************************************************************************************/
	...
	unsigned int i,j,k,m,p, random_num;
	...
	**********************************************************/
	...
	random_num = ssd->token;  /*产生一个随机数，保证每次从不同的channel开始查询*/

	
	for (unsigned int chan = 0; chan<ssd->parameter->channel_number; chan++)
	{
		...
		//找到当前状态为CHANNEL_IDLE或者下一状态为CHANNEL_IDEL并且下一状态预计时间小于当前系统时间的channel
		if ((ssd->channel_head[i].current_state == CHANNEL_IDLE) || (ssd->channel_head[i].next_state == CHANNEL_IDLE&&ssd->channel_head[i].next_state_predict_time <= ssd->current_time))
		{
			...
			**********************************************************/
			...
			//如果channel[i]为空闲状态，并且子请求序列非空，则处理
		...
		else
		{
			...
			if (flag == 1)  //该请求的所有子请求都已经完成
			{
				...
				while (req->subs != NULL)
				{
					...
					if (tmp->update != NULL) //该子请求产生更新读请求，需要释放掉更新读请求的空间
					{
