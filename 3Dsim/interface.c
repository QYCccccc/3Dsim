...
int get_requests(struct ssd_info *ssd)
{
	...
	unsigned int approxFlag = 0;  //近似标签
	struct request *request1;
	int flag = 1;
	long filepoint;
	__int64 time_t;	//trace到达时间
	...
	while (TRUE)
	{
		//是否预处理flash, 0:warmflash, 1:non-warmflash
...
__int64 find_nearest_event(struct ssd_info *ssd)
{
	...
	for (i = 0; i<ssd->parameter->channel_number; i++)
	{
		if (ssd->channel_head[i].next_state == CHANNEL_IDLE)
			if (time1>ssd->channel_head[i].next_state_predict_time) //下一状态可到达，或下一状态预计时间小于之前记录的最小值
				if (ssd->channel_head[i].next_state_predict_time>ssd->current_time) //IO非阻塞，时间顺序性正常
