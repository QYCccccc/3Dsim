...
3.调用go_one_step()函数将子请求转移到目标状态SR_R_READ
**********************************************************************************************/
Status services_2_r_read(struct ssd_info * ssd)
{
	...
	for (i = 0; i < ssd->parameter->channel_number; i++)                                    
	{
		...
		{
			/*只有当chip的状态为READ_BUSY或者下一状态为READ_BUSY时，
			下一状态为SR_R_READ的子请求才能被处理*/
				...
				(ssd->channel_head[i].chip_head[j].next_state_predict_time <= ssd->current_time)))
			{
				...
				{					
					...
					//找出符合位置条件，以及其下一状态为SR_R_READ并且下一状态预计时间小于当前时间的sub_requeset
...
*在channel的读子请求序列中找到下一状态为state,并且下一状态预计时间小于当前系统时间的子请求
*保存在subs数组里
*返回值为符合条件的sub_request的数量
*/
...
******************************************************************************************/
/**
 *1. 找出读子请求中当前状态为SR_WAIT,并且访问的chip为参数中给定的chip，
 *读取子请求访问的chip当前状态为CHIP_IDLE或者下一状态为CHIP_IDLE并且下一状态预计时间小于当前系统时间
 *2.调用go_one_step()函数将子请求状态转移到目标状态SR_R_C_A_TRANSFER
 */
Status services_2_r_wait(struct ssd_info * ssd, unsigned int channel)
{
	struct sub_request ** sub_place = NULL;
	unsigned int i ,chip;
	unsigned int sub_r_req_count = 0;
...
/**
 *1. 找出读子请求中当前状态为SR_WAIT,并且访问的chip为参数中给定的chip
 *2.读取子请求访问的chip当前状态为CHIP_IDLE或者下一状态为CHIP_IDLE并且下一状态预计时间小于当前系统时间
 */
...
*1.子请求的状态转移和时间的计算由该功能处理（即转移到目标状态）
*2.正常命令的执行状态和时间的计算由该功能处理
*/
